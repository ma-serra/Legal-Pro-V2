...
from sqlalchemy import Column, Integer, String, Text, Boolean, DateTime, ForeignKey, Table, UniqueConstraint, Index, Float, JSON
...
class ValidacaoMultiAgenteAnalise(db.Model):
    ...
    # Dados de exportação
    exportado_word = Column(Boolean, default=False)
    exportado_pdf = Column(Boolean, default=False)
    exportado_json = Column(Boolean, default=False)
    data_ultima_exportacao = Column(DateTime, nullable=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'exportado_word': self.exportado_word,
            'exportado_pdf': self.exportado_pdf,
            'exportado_json': self.exportado_json,
            'data_ultima_exportacao': self.data_ultima_exportacao.isoformat() if self.data_ultima_exportacao else None
        }
    ...
    def marcar_exportacao(self, formato):
        ...
        if formato == 'word':
            self.exportado_word = True
        elif formato == 'pdf':
            self.exportado_pdf = True
        elif formato == 'json':
            self.exportado_json = True
        
        self.data_ultima_exportacao = datetime.datetime.now()
        db.session.commit()
...
class ProcessoJuridico(db.Model):
    ...
    estimativa_desembolso = Column(Float, nullable=True)
    ...
    bloqueios = Column(Float, nullable=True)
    ...
    def to_dict(self):
        ...
        return {
            ...
            'estimativa_desembolso': self.estimativa_desembolso,
