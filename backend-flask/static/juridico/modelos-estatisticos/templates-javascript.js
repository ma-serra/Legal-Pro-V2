...
const MATRIZ_AREAS_JURIDICAS = {
    ...
    'agrario': {
        ...
        precisao_media: 88.5,
        campos_especificos: [
            'desapropriacao_rural', 'reforma_agraria',
            'usucapiao_rural', 'arrendamento_rural',
            'parceria_agricola', 'credito_rural',
            'seguro_rural', 'politica_agricola',
            'meio_ambiente_rural', 'trabalho_rural',
            'cooperativismo_agricola', 'modulo_rural'
        ],
        caracteristicas: 'Legislação específica rural'
    }
};
...
function atualizarCamposEspecificosArea(area, modelType = 'regressao') {
    ...
    const areaConfig = MATRIZ_AREAS_JURIDICAS[area];
    if (!areaConfig) {
        console.log(`⚠️ Configuração não encontrada para área ${area}`);
        return;
    }
    
    // Encontrar container apropriado baseado no modelo
    const containerIds = {
        'regressao': 'camposEspecificosArea',
        'arvore': 'camposEspecificosAreaArvore',
        'neurais': 'camposEspecificosAreaNeurais',
        'temporais': 'camposEspecificosAreaTemporais',
        'sobrevivencia': 'camposEspecificosAreaSobrevivencia'
    };
    
    const containerId = containerIds[modelType] || 'camposEspecificosArea';
    const container = document.getElementById(containerId);
    
    if (!container) {
        console.log(`⚠️ Container ${containerId} não encontrado para modelo ${modelType}`);
        return;
    }
        ...
        <div class="row mt-3">
            <div class="col-12">
                <div class="alert alert-info mb-0">
                    ...
                    Volume mínimo recomendado: <strong>${formatarNumero(areaConfig.volume_minimo)}</strong> casos 
    ...
    console.log(`✓ Campos específicos atualizados para área '${area}' no modelo '${modelType}'`);
}
...
// ==================== FUNÇÕES AUXILIARES PARA REDES NEURAIS ====================
...
function formatarNumero(numero) {
    ...
    // Usar formato brasileiro com pontos para milhares
    return numero.toLocaleString('pt-BR');
}

/**
 * Mostrar aviso temporário para o usuário
 * @param {string} mensagem - Mensagem do aviso
 * @param {string} tipo - Tipo do aviso (info, warning, error, success)
 */
function mostrarAviso(mensagem, tipo = 'info') {
    // Criar elemento de aviso se não existir
    let avisoContainer = document.getElementById('aviso-container');
    if (!avisoContainer) {
        avisoContainer = document.createElement('div');
        avisoContainer.id = 'aviso-container';
        avisoContainer.style.cssText = `
            position: fixed;
            top: 20px;
            right: 20px;
            z-index: 9999;
    ...
    aviso.className = `alert alert-${tipo === 'error' ? 'danger' : tipo} alert-dismissible fade show`;
    aviso.style.cssText = 'margin-bottom: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.2);';
...
function adicionarCamada() {
    ...
    div.innerHTML = `
        <div class="row">
            <div class="col-4">
                <select class="form-select" name="tipoCamada[]" onchange="atualizarConfigCamada(this)">
                    <optgroup label="🏗️ CAMADAS BÁSICAS">
                    ...
                    <optgroup label="🔄 CAMADAS RECORRENTES">
                        <option value="lstm">LSTM</option>
                        <option value="gru">GRU</option>
                        <option value="simple_rnn">Simple RNN</option>
                        <option value="bidirectional_lstm">Bidirectional LSTM</option>
                        <option value="bidirectional_gru">Bidirectional GRU</option>
                    </optgroup>
                    <optgroup label="📋 CAMADAS CONVOLUCIONAIS">
                        <option value="conv1d">Conv1D</option>
                        <option value="conv2d">Conv2D</option>
                        <option value="separable_conv1d">Separable Conv1D</option>
                        <option value="depthwise_conv2d">Depthwise Conv2D</option>
                    </optgroup>
                    <optgroup label="⚖️ CAMADAS ESPECIALIZADAS JURÍDICAS">
                        <option value="attention_juridico">Attention Jurídico</option>
                        <option value="self_attention">Self-Attention</option>
                        <option value="multi_head_attention">Multi-Head Attention</option>
                        <option value="transformer_encoder">Transformer Encoder</option>
                        <option value="bert_encoder">BERT Encoder</option>
                        <option value="legal_embedding">Legal Embedding</option>
                    </optgroup>
                    <optgroup label="🛡️ CAMADAS DE REGULARIZAÇÃO">
                        <option value="dropout">Dropout</option>
                        <option value="spatial_dropout">Spatial Dropout</option>
                        <option value="batch_norm">Batch Normalization</option>
                        <option value="layer_norm">Layer Normalization</option>
                        <option value="instance_norm">Instance Normalization</option>
                    </optgroup>
                    <optgroup label="📊 CAMADAS DE POOLING">
                        <option value="max_pool1d">Max Pooling 1D</option>
                        <option value="avg_pool1d">Average Pooling 1D</option>
                        <option value="global_max_pool">Global Max Pooling</option>
                        <option value="global_avg_pool">Global Average Pooling</option>
                    </optgroup>
                </select>
            </div>
            <div class="col-3">
                <input type="number" class="form-control" placeholder="Neurônios" name="numeroNeuronios[]" 
                       min="1" max="4096" onchange="validarNumeroNeuronios(this)">
            </div>
            <div class="col-4">
                <select class="form-select" name="ativacao[]">
                    <optgroup label="🔥 ATIVAÇÕES BÁSICAS">
                    ...
                    <optgroup label="⚡ ATIVAÇÕES AVANÇADAS">
                    ...
                    <optgroup label="🎯 ATIVAÇÕES ESPECIALIZADAS">
                        <option value="glu">GLU (Gated Linear Unit)</option>
    ...
    `;
    container.appendChild(div);
    atualizarContagemCamadas();
}

function removerCamada(button) {
    // Verificar se o botão e o elemento pai existem
    if (!button) {
        console.error('❌ Botão não encontrado para remoção de camada');
        return;
    }
    
    const camadaDiv = button.closest('.camada');
    if (!camadaDiv) {
        console.error('❌ Elemento camada não encontrado');
        return;
    }
    
    // Remover com segurança
    camadaDiv.remove();
    console.log('✓ Camada removida com sucesso');
...
// Funções auxiliares para Redes Neurais
function atualizarContagemCamadas() {
    const camadas = document.querySelectorAll('#camadasContainer .camada');
    const contador = document.getElementById('contadorCamadas');
    if (contador) {
        contador.textContent = `${camadas.length} camadas configuradas`;
    }
    
    // Atualizar estimativa de complexidade
    atualizarEstimativaComplexidade();
}

function validarNumeroNeuronios(input) {
    const valor = parseInt(input.value);
    const tipo = input.closest('.row').querySelector('select[name="tipoCamada[]"]').value;
    
    // Definir limites por tipo de camada
    const limitesPorTipo = {
        'embedding': { min: 50, max: 1024, recomendado: 300 },
        'dense': { min: 1, max: 4096, recomendado: 256 },
        'lstm': { min: 10, max: 1024, recomendado: 128 },
        'gru': { min: 10, max: 1024, recomendado: 128 },
        'conv1d': { min: 1, max: 512, recomendado: 64 },
        'conv2d': { min: 1, max: 512, recomendado: 32 },
        'attention_juridico': { min: 64, max: 1024, recomendado: 512 },
        'transformer_encoder': { min: 128, max: 2048, recomendado: 768 },
        'bert_encoder': { min: 256, max: 2048, recomendado: 768 }
    };
    
    const limites = limitesPorTipo[tipo] || { min: 1, max: 4096, recomendado: 256 };
    
    if (valor < limites.min || valor > limites.max) {
        input.classList.add('is-invalid');
        mostrarAviso(`Número de neurônios para ${tipo} deve estar entre ${limites.min} e ${limites.max}. Recomendado: ${limites.recomendado}`);
    } else {
...
function atualizarConfigCamada(select) {
    const row = select.closest('.row');
    const inputNeuronios = row.querySelector('input[name="numeroNeuronios[]"]');
    const selectAtivacao = row.querySelector('select[name="ativacao[]"]');
    const tipo = select.value;
    
    // Configurações padrão por tipo de camada
    const configPadrao = {
        'embedding': { neuronios: 300, ativacao: 'linear', placeholder: 'Dimensão embedding' },
        'dense': { neuronios: 256, ativacao: 'relu', placeholder: 'Neurônios' },
        'lstm': { neuronios: 128, ativacao: 'tanh', placeholder: 'Unidades LSTM' },
        'gru': { neuronios: 128, ativacao: 'tanh', placeholder: 'Unidades GRU' },
        'conv1d': { neuronios: 64, ativacao: 'relu', placeholder: 'Filtros' },
        'conv2d': { neuronios: 32, ativacao: 'relu', placeholder: 'Filtros' },
        'dropout': { neuronios: '', ativacao: 'linear', placeholder: 'Taxa (0.1-0.9)' },
        'batch_norm': { neuronios: '', ativacao: 'linear', placeholder: 'N/A' },
        'attention_juridico': { neuronios: 512, ativacao: 'softmax', placeholder: 'Dimensão atenção' },
        'transformer_encoder': { neuronios: 768, ativacao: 'gelu', placeholder: 'Dimensão modelo' },
        'bert_encoder': { neuronios: 768, ativacao: 'gelu', placeholder: 'Hidden size' }
    };
    
    const config = configPadrao[tipo] || { neuronios: 256, ativacao: 'relu', placeholder: 'Neurônios' };
    
    if (inputNeuronios) {
        inputNeuronios.value = config.neuronios;
        inputNeuronios.placeholder = config.placeholder;
        
        // Desabilitar input para camadas que não precisam de neurônios
        if (tipo === 'dropout' || tipo === 'batch_norm' || tipo.includes('norm')) {
            inputNeuronios.disabled = true;
        } else {
            inputNeuronios.disabled = false;
        }
    }
    
    if (selectAtivacao) {
        selectAtivacao.value = config.ativacao;
    }
}

function atualizarEstimativaComplexidade() {
    const camadas = document.querySelectorAll('#camadasContainer .camada');
    let complexidadeTotal = 0;
    let parametrosEstimados = 0;
    
    camadas.forEach((camada, index) => {
        const tipo = camada.querySelector('select[name="tipoCamada[]"]').value;
        const neuronios = parseInt(camada.querySelector('input[name="numeroNeuronios[]"]').value) || 0;
        
        // Estimativa simples de complexidade por tipo
        const complexidadePorTipo = {
            'dense': neuronios * 2,
            'lstm': neuronios * 4,
            'gru': neuronios * 3,
            'conv1d': neuronios * 1.5,
            'conv2d': neuronios * 2,
            'attention_juridico': neuronios * 3,
            'transformer_encoder': neuronios * 5,
            'bert_encoder': neuronios * 6,
            'dropout': 0,
            'batch_norm': neuronios * 0.1
        };
        
        complexidadeTotal += complexidadePorTipo[tipo] || neuronios;
        
        // Estimativa de parâmetros (simplificada)
        if (index === 0) {
            parametrosEstimados += neuronios * 100; // Input típico
        } else {
            const camadaAnterior = camadas[index - 1];
            const neuroniosAnterior = parseInt(camadaAnterior.querySelector('input[name="numeroNeuronios[]"]').value) || 256;
            parametrosEstimados += neuronios * neuroniosAnterior;
        }
    });
    
    // Atualizar displays na interface
    const complexidadeDisplay = document.getElementById('complexidadeEstimada');
    const parametrosDisplay = document.getElementById('parametrosEstimados');
    
    if (complexidadeDisplay) {
        complexidadeDisplay.textContent = formatarNumero(complexidadeTotal);
    }
    
    if (parametrosDisplay) {
        parametrosDisplay.textContent = formatarNumero(parametrosEstimados);
    }
    
    // Atualizar recomendação de volume mínimo baseado na complexidade
    atualizarVolumeRecomendadoNeurais(complexidadeTotal);
}

function atualizarVolumeRecomendadoNeurais(complexidade) {
    const volumeMinimoElement = document.getElementById('volumeMinimoNeurais');
    if (!volumeMinimoElement) return;
    
    // Fórmula: Volume mínimo = complexidade * 10 (regra empírica)
    let volumeMinimo = Math.max(1000, complexidade * 10);
    ...
    const multiplicador = multiplicadores[arquitetura] || 10;
    volumeMinimo = Math.max(1000, complexidade * multiplicador);
    
    volumeMinimoElement.textContent = formatarNumero(volumeMinimo);
    
    // Atualizar status baseado no volume atual
    const volumeAtual = parseInt(document.getElementById('volumeAtualNeurais')?.textContent?.replace(/\D/g, '') || '0');
    atualizarStatusVolumeNeurais(volumeAtual, volumeMinimo);
}

function atualizarStatusVolumeNeurais(volumeAtual, volumeMinimo) {
    const statusElement = document.getElementById('statusVolumeNeurais');
    const alertElement = document.getElementById('alertStatusVolumeNeurais');
    const progressElement = document.getElementById('progressoVolumeNeurais');
    
    if (!statusElement || !alertElement || !progressElement) return;
    
    const percentual = Math.min(100, (volumeAtual / volumeMinimo) * 100);
    
    if (volumeAtual >= volumeMinimo) {
        statusElement.textContent = 'Volume Adequado';
        alertElement.className = 'alert alert-success mb-0';
        progressElement.className = 'progress-bar bg-success';
        progressElement.style.width = '100%';
        progressElement.textContent = '100% - Adequado';
    } else if (volumeAtual >= volumeMinimo * 0.7) {
        statusElement.textContent = 'Volume Aceitável';
        ...
        progressElement.textContent = Math.round(percentual) + '% - Aceitável';
    } else {
        statusElement.textContent = 'Volume Insuficiente';
        alertElement.className = 'alert alert-danger mb-0';
        progressElement.className = 'progress-bar bg-danger';
        progressElement.style.width = percentual + '%';
        progressElement.textContent = Math.round(percentual) + '% - Insuficiente';
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.atualizarParametrosSobrevivencia = function() {
        const area = document.getElementById('areaJuridicaModeloSobrevivencia')?.value || 'civil';
        // Análise de sobrevivência: suporta todas as áreas
...
// ==================== FUNÇÕES PARA INTEGRAÇÃO POSTGRESQL ====================

/**
 * Função para listar tabelas disponíveis no PostgreSQL
 * @param {string} selectId - ID do select onde as tabelas serão carregadas
 * @param {function} callback - Função de callback opcional após carregar
 */
window.carregarTabelasPostgreSQL = async function(selectId, callback) {
    ...
    try {
        // Mostrar loading
        selectElement.innerHTML = '<option value="">🔄 Carregando tabelas...</option>';
        selectElement.disabled = true;

        const response = await fetch('/api/database/tabelas');
        const data = await response.json();

        if (data.success && data.tabelas) {
            // Limpar select
            selectElement.innerHTML = '<option value="">📋 Selecione uma tabela</option>';
            
            // Adicionar tabelas
            data.tabelas.forEach(tabela => {
                const option = document.createElement('option');
                option.value = tabela.nome;
                option.textContent = `${tabela.nome} (${tabela.tipo})`;
                option.setAttribute('data-tipo', tabela.tipo_raw);
            ...
            if (callback && typeof callback === 'function') {
                callback(data.tabelas);
            }

            console.log(`✓ ${data.total} tabelas carregadas no select ${selectId}`);
        } else {
        ...
        selectElement.innerHTML = '<option value="">❌ Erro ao carregar tabelas</option>';
        selectElement.disabled = false;
 ...
 * @param {string} selectId - ID do select onde as colunas serão carregadas
 * @param {function} callback - Função de callback opcional após carregar
 */
window.carregarColunasTabela = async function(nomeTabela, selectId, callback) {
    const selectElement = document.getElementById(selectId);
    if (!selectElement) {
        console.error('Elemento select não encontrado:', selectId);
        return;
    ...
    if (!nomeTabela) {
        selectElement.innerHTML = '<option value="">📋 Selecione uma coluna</option>';
        selectElement.disabled = true;
        return;
    }
    ...
    try {
        // Mostrar loading
        selectElement.innerHTML = '<option value="">🔄 Carregando colunas...</option>';
        selectElement.disabled = true;

        const response = await fetch(`/api/database/colunas/${encodeURIComponent(nomeTabela)}`);
        const data = await response.json();

        if (data.success && data.colunas) {
            // Limpar select
            selectElement.innerHTML = '<option value="">📋 Selecione uma coluna</option>';
            ...
            data.colunas.forEach(coluna => {
                ...
                option.textContent = `${coluna.nome} (${coluna.tipo})`;
                option.setAttribute('data-tipo', coluna.tipo);
                option.setAttribute('data-nulo', coluna.nulo);
                selectElement.appendChild(option);
            });
            ...
            // Executar callback se fornecido
            if (callback && typeof callback === 'function') {
                callback(data.colunas);
            }

            console.log(`✓ ${data.total} colunas carregadas da tabela ${nomeTabela}`);
        } else {
            throw new Error(data.message || 'Erro ao carregar colunas');
        }
        ...
        if (typeof showAlert === 'function') {
            ...
            alert('Erro ao obter informações da tabela: ' + error.message);
        }
    }
};

/**
 * Função para visualizar preview dos dados de uma tabela
 * @param {string} nomeTabela - Nome da tabela
 * @param {string} containerId - ID do container onde o preview será exibido
 */
window.visualizarPreviewTabela = async function(nomeTabela, containerId) {
    ...
    if (!nomeTabela) {
        containerElement.innerHTML = '<div class="alert alert-info">📋 Selecione uma tabela para visualizar os dados</div>';
        return;
    }

    try {
        // Mostrar loading
        containerElement.innerHTML = '<div class="alert alert-info">🔄 Carregando dados...</div>';

        const response = await fetch(`/api/database/preview/${encodeURIComponent(nomeTabela)}`);
        const data = await response.json();
        ...
        if (data.success && data.dados) {
            let html = `
                <div class="card">
                    <div class="card-header">
                        <h6 class="mb-0">📊 Preview da Tabela: ${data.tabela}</h6>
                        <small class="text-muted">Mostrando ${data.total_registros} primeiros registros</small>
                    </div>
                    <div class="card-body">
            `;
            ...
            if (data.dados.length > 0) {
                ...
                html += '<thead><tr>';
                data.colunas.forEach(coluna => {
                    html += `<th style="white-space: nowrap;">${coluna}</th>`;
                });
                html += '</tr></thead>';
                ...
                data.dados.forEach(registro => {
                    ...
                    data.colunas.forEach(coluna => {
                        ...
                        const valorFormatado = valor === null || valor === undefined ? 
                            ...
                            String(valor).length > 50 ? String(valor).substring(0, 50) + '...' : String(valor);
                        html += `<td style="max-width: 200px; overflow: hidden;">${valorFormatado}</td>`;
                    });
                    html += '</tr>';
                });
                html += '</tbody></table></div>';
            } else {
                html += '<div class="alert alert-warning">⚠️ Tabela vazia - nenhum registro encontrado</div>';
            }

            html += '</div></div>';
            containerElement.innerHTML = html;

            console.log(`✓ Preview da tabela ${nomeTabela} carregado com ${data.total_registros} registros`);
        } else {
        ...
        containerElement.innerHTML = `<div class="alert alert-danger">❌ Erro ao carregar dados: ${error.message}</div>`;
    }
};

/**
 * Função para configurar seleção automática de tabela com colunas
 * @param {string} tabelaSelectId - ID do select de tabelas
 * @param {string} colunaSelectId - ID do select de colunas
 * @param {string} previewContainerId - ID do container de preview (opcional)
 */
window.configurarSelecaoTabelaColunas = function(tabelaSelectId, colunaSelectId, previewContainerId) {
    const tabelaSelect = document.getElementById(tabelaSelectId);
    const colunaSelect = document.getElementById(colunaSelectId);

    if (!tabelaSelect || !colunaSelect) {
        console.error('Elementos select não encontrados:', tabelaSelectId, colunaSelectId);
        return;
    }

    // Carregar tabelas inicialmente
    window.carregarTabelasPostgreSQL(tabelaSelectId);

    // Configurar evento de mudança de tabela
    tabelaSelect.addEventListener('change', function() {
        const tabelaSelecionada = this.value;
        
        // Carregar colunas da tabela selecionada
        window.carregarColunasTabela(tabelaSelecionada, colunaSelectId);
        
        // Mostrar preview se container foi fornecido
        if (previewContainerId) {
            window.visualizarPreviewTabela(tabelaSelecionada, previewContainerId);
        }
    });

    console.log(`✓ Seleção de tabela/colunas configurada: ${tabelaSelectId} -> ${colunaSelectId}`);
};

/**
 * Função para adicionar componente de seleção de tabela PostgreSQL em um container
 * @param {string} containerId - ID do container onde o componente será adicionado
 * @param {object} options - Opções de configuração
 */
window.adicionarSeletorTabelaPostgreSQL = function(containerId, options = {}) {
    const container = document.getElementById(containerId);
    if (!container) {
        console.error('Container não encontrado:', containerId);
        return;
    ...
    const config = {
        ...
        showColumns: options.showColumns !== false,
        tabelaLabel: options.tabelaLabel || 'Tabela PostgreSQL',
        colunaLabel: options.colunaLabel || 'Coluna de Dados',
        tabelaId: options.tabelaId || `tabela_${Date.now()}`,
        colunaId: options.colunaId || `coluna_${Date.now()}`,
        previewId: options.previewId || `preview_${Date.now()}`,
        ...options
    };
    ...
    let html = `
        <div class="card mt-3">
            <div class="card-header">
                <h6 class="mb-0">🗃️ Seleção de Dados PostgreSQL</h6>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <label for="${config.tabelaId}" class="form-label">${config.tabelaLabel}</label>
                        <select id="${config.tabelaId}" class="form-select">
    ...
    if (config.showColumns) {
        html += `
                    <div class="col-md-6">
                        <label for="${config.colunaId}" class="form-label">${config.colunaLabel}</label>
                        <select id="${config.colunaId}" class="form-select" disabled>
                            <option value="">📋 Selecione uma tabela primeiro</option>
                        </select>
                    </div>
        `;
    }
    ...
    if (config.showPreview) {
        html += `
                <div class="mt-3">
                    <div id="${config.previewId}">
                        <div class="alert alert-info">📋 Selecione uma tabela para visualizar os dados</div>
                    </div>
                </div>
        `;
    }
    ...
    setTimeout(() => {
        if (config.showColumns) {
            window.configurarSelecaoTabelaColunas(
                config.tabelaId, 
                config.colunaId, 
                config.showPreview ? config.previewId : null
            );
        } else {
            window.carregarTabelasPostgreSQL(config.tabelaId);
            if (config.showPreview) {
                document.getElementById(config.tabelaId).addEventListener('change', function() {
                    window.visualizarPreviewTabela(this.value, config.previewId);
                });
            }
        }
    }, 100);

    console.log(`✓ Seletor PostgreSQL adicionado ao container ${containerId}`);
    return config;
};

// ==================== FUNÇÕES ESPECÍFICAS PARA SÉRIES TEMPORAIS ====================

function atualizarParametrosTemporais() {
    const areaJuridica = document.getElementById('areaJuridicaModeloTemporais')?.value;
    console.log(`🔄 Atualizando parâmetros temporais para área: ${areaJuridica}`);
    
    // Ocultar todas as seções de campos específicos
    const secoesEspecificas = [
        'camposTrabalhistaTemporais',
        'camposCivelTemporais', 
        'camposTributarioTemporais',
        'camposConsumidorTemporais',
        'camposFamiliaTemporais',
        'camposPrevidenciarioTemporais',
        'camposPenalTemporais'
    ];
    
    secoesEspecificas.forEach(id => {
    ...
    // Mostrar seção específica da área selecionada
    const mapeamentoAreas = {
        'trabalhista': 'camposTrabalhistaTemporais',
        'civil': 'camposCivelTemporais',
        'tributario': 'camposTributarioTemporais',
        'consumidor': 'camposConsumidorTemporais',
        'familia': 'camposFamiliaTemporais',
        'previdenciario': 'camposPrevidenciarioTemporais',
        'penal': 'camposPenalTemporais'
    };
    
    if (mapeamentoAreas[areaJuridica]) {
        const elementoEspecifico = document.getElementById(mapeamentoAreas[areaJuridica]);
        if (elementoEspecifico) {
            elementoEspecifico.style.display = 'block';
            console.log(`✓ Campos específicos exibidos para: ${areaJuridica}`);
        }
    }
}

function atualizarParametrosModeloTemporal() {
    const tipoAnalise = document.getElementById('tipoAnaliseTemporaisEdit')?.value;
    console.log(`🔄 Atualizando parâmetros do modelo temporal: ${tipoAnalise}`);
    
    const parametrosARIMA = document.getElementById('parametrosARIMA');
    if (parametrosARIMA) {
        // Mostrar parâmetros ARIMA apenas para modelos ARIMA e SARIMA
        if (tipoAnalise === 'arima' || tipoAnalise === 'sarima_sazonal' || tipoAnalise === 'arfima') {
            parametrosARIMA.style.display = 'block';
            console.log('✓ Parâmetros ARIMA/SARIMA exibidos');
        } else {
            parametrosARIMA.style.display = 'none';
            console.log('✓ Parâmetros ARIMA/SARIMA ocultados');
        }
    }
    
    // Atualizar configurações específicas baseadas no modelo
    if (tipoAnalise === 'prophet_facebook') {
        console.log('⚡ Configurações otimizadas para Prophet');
        atualizarConfiguracoesProphet();
    } else if (tipoAnalise.includes('lstm') || tipoAnalise.includes('gru') || tipoAnalise.includes('transformer')) {
        console.log('⚡ Configurações otimizadas para Deep Learning');
        atualizarConfiguracoesDeepLearning();
    }
}

function atualizarConfiguracoesProphet() {
    // Configurações específicas para Prophet
    const seasonalPeriod = document.getElementById('seasonalPeriodTemporais');
    if (seasonalPeriod) {
        seasonalPeriod.value = '365'; // Prophet funciona bem com sazonalidade diária/anual
    }
    
    const windowSize = document.getElementById('windowSizeTemporais');
    if (windowSize) {
        windowSize.value = '365'; // Prophet precisa de mais dados históricos
    }
}

function atualizarConfiguracoesDeepLearning() {
    // Configurações específicas para modelos de Deep Learning
    const lagFeatures = document.getElementById('lagFeaturesTemporais');
    if (lagFeatures) {
        lagFeatures.value = '24'; // Mais lag features para deep learning
    }
    
    const windowSize = document.getElementById('windowSizeTemporais');
    if (windowSize) {
        windowSize.value = '60'; // Window size maior para LSTM/GRU
    }
}

function validarPeriodoHistorico() {
    const periodoHistorico = document.getElementById('periodoHistorico')?.value;
    const tipoAnalise = document.getElementById('tipoAnaliseTemporaisEdit')?.value;
    
    // Extrair número de meses do período
    let mesesDisponiveis = 0;
    if (periodoHistorico.includes('_meses')) {
        mesesDisponiveis = parseInt(periodoHistorico.split('_')[0]);
    } else if (periodoHistorico.includes('_anos') || periodoHistorico.includes('_ano')) {
        const anos = parseInt(periodoHistorico.split('_')[0]);
        mesesDisponiveis = anos * 12;
    }
    
    // Atualizar elementos de validação
    const periodoAtualElement = document.getElementById('periodoAtualTemporais');
    const statusPeriodoElement = document.getElementById('statusPeriodoTemporais');
    const progressoElement = document.getElementById('progressoPeriodoTemporais');
    const alertStatusElement = document.getElementById('alertStatusPeriodoTemporais');
    const impactoElement = document.getElementById('impactoTemporalTemporais');
    
    if (periodoAtualElement) periodoAtualElement.textContent = mesesDisponiveis;
    
    const periodoMinimo = 24; // Mínimo 2 anos
    const progresso = Math.min((mesesDisponiveis / periodoMinimo) * 100, 100);
    
    if (progressoElement) {
        progressoElement.style.width = `${progresso}%`;
        progressoElement.textContent = `${Math.round(progresso)}%`;
        
        if (progresso >= 100) {
            progressoElement.className = 'progress-bar bg-success';
        } else if (progresso >= 75) {
            progressoElement.className = 'progress-bar bg-warning';
        } else {
            progressoElement.className = 'progress-bar bg-danger';
        }
    }
    
    if (statusPeriodoElement && alertStatusElement && impactoElement) {
        if (mesesDisponiveis >= periodoMinimo) {
            statusPeriodoElement.textContent = '✓ Adequado';
            alertStatusElement.className = 'alert mb-0 alert-success';
            impactoElement.textContent = 'Período adequado para análise sazonal completa';
        } else if (mesesDisponiveis >= 12) {
            statusPeriodoElement.textContent = '⚠️ Limitado';
            alertStatusElement.className = 'alert mb-0 alert-warning';
            impactoElement.textContent = 'Sazonalidade anual pode ser incompleta';
        } else {
            statusPeriodoElement.textContent = '❌ Insuficiente';
            alertStatusElement.className = 'alert mb-0 alert-danger';
            impactoElement.textContent = 'Período insuficiente - pode perder padrões sazonais';
        }
    }
    
    console.log(`📊 Validação período: ${mesesDisponiveis} meses (${Math.round(progresso)}% adequação)`);
}

// Inicialização das funções de séries temporais
document.addEventListener('DOMContentLoaded', function() {
    console.log('🎯 Inicializando funções específicas de Séries Temporais');
    
    // Configurar eventos para atualização de parâmetros
    const areaSelect = document.getElementById('areaJuridicaModeloTemporais');
    if (areaSelect) {
        areaSelect.addEventListener('change', atualizarParametrosTemporais);
        console.log('✓ Event listener configurado para área jurídica');
    }
    
    const tipoAnaliseSelect = document.getElementById('tipoAnaliseTemporaisEdit');
    if (tipoAnaliseSelect) {
        tipoAnaliseSelect.addEventListener('change', atualizarParametrosModeloTemporal);
        console.log('✓ Event listener configurado para tipo de análise');
    }
    
    const periodoSelect = document.getElementById('periodoHistorico');
    if (periodoSelect) {
        periodoSelect.addEventListener('change', validarPeriodoHistorico);
        console.log('✓ Event listener configurado para validação de período');
        
        // Executar validação inicial
        setTimeout(validarPeriodoHistorico, 500);
    }
    
    // Configurar formatação monetária para campos de valor
    const camposMonetarios = ['valorDaCausaTemporais'];
    camposMonetarios.forEach(campoId => {
        ...
        if (campo) {
            ...
            console.log(`✓ Formatação monetária configurada para: ${campoId}`);
        }
    });
    
    console.log('🎉 Inicialização de Séries Temporais concluída!');
});

// ==================== FUNÇÕES PARA BASES VETORIAIS QDRANT ====================

/**
 * Carrega collections disponíveis no Qdrant
 * @param {string} selectId - ID do select onde as collections serão carregadas
 * @param {function} callback - Função de callback opcional após carregar
 */
window.carregarCollectionsQdrant = async function(selectId, callback) {
    const selectElement = document.getElementById(selectId);
    if (!selectElement) {
        console.error('❌ Elemento select não encontrado:', selectId);
        return;
    ...
    try {
        console.log(`🔄 Carregando collections do Qdrant para select ${selectId}...`);
        selectElement.innerHTML = '<option value="">🔄 Carregando collections...</option>';
        selectElement.disabled = true;

        console.log('📡 Fazendo fetch para /api/qdrant/collections...');
        ...
        if (data.success && data.collections) {
            console.log(`✅ ${data.collections.length} collections carregadas do Qdrant`);
            
            // Limpar e adicionar opção vazia
            selectElement.innerHTML = '<option value="">📦 Selecione uma collection</option>';
            
            // Agrupar collections por área jurídica
            const collectionsPorArea = {};
            data.collections.forEach(collection => {
                const area = collection.area_juridica || collection.area || 'Geral';
                if (!collectionsPorArea[area]) {
                    collectionsPorArea[area] = [];
                }
                collectionsPorArea[area].push(collection);
            });
            
            // Adicionar optgroups por área
            Object.keys(collectionsPorArea).sort().forEach(area => {
                const optgroup = document.createElement('optgroup');
                optgroup.label = `${area} (${collectionsPorArea[area].length} collections)`;
                
                collectionsPorArea[area].forEach(collection => {
                    const option = document.createElement('option');
                    option.value = collection.nome || collection.name || collection.collection_name || 'collection_sem_nome';
                    
                    // Tratamento seguro para exibição
                    const nome = collection.nome || collection.name || 'Collection';
                    const pontos = collection.pontos || collection.points_count || 0;
                    const icone = collection.icone || '📦';
                    
                    option.textContent = `${icone} ${nome} (${pontos} pontos)`;
                    option.setAttribute('data-type', collection.tipo || collection.type || 'geral');
                    option.setAttribute('data-points', pontos);
                    option.setAttribute('data-area', collection.area_juridica || collection.area || 'Geral');
                    option.setAttribute('data-distance', 'Cosine');
                    option.setAttribute('data-size', 'Variable');
            ...
            if (callback) callback(data.collections);
            
        } else {
            console.error('❌ Erro ao carregar collections:', data.error);
            selectElement.innerHTML = '<option value="">❌ Erro ao carregar collections</option>';
            mostrarErro(data.message || 'Erro ao conectar com a base vetorial Qdrant');
        }
    } catch (error) {
        console.error('❌ Erro de rede ao carregar collections:', error);
        selectElement.innerHTML = '<option value="">❌ Erro de conexão</option>';
        mostrarErro('Erro de conexão com a base vetorial Qdrant');
    }
};

/**
 * Visualiza informações de uma collection específica
 * @param {string} collectionName - Nome da collection
 * @param {string} containerId - ID do container onde exibir as informações
 */
window.visualizarInfoCollection = async function(collectionName, containerId) {
    const container = document.getElementById(containerId);
    if (!container) {
        console.error('Container não encontrado:', containerId);
        return;
    ...
    if (!collectionName) {
        container.innerHTML = `
            <div class="alert alert-info">
                <i class="fas fa-info-circle me-2"></i>
                Selecione uma collection para visualizar informações detalhadas
            </div>
        `;
        return;
    }
    ...
    try {
        ...
        container.innerHTML = '<div class="alert alert-info">🔄 Carregando informações da collection...</div>';

        const response = await fetch(`/api/qdrant/collection/${encodeURIComponent(collectionName)}/info`);
        const data = await response.json();

        if (data.success && data.collection_info) {
            const info = data.collection_info;
            
            let fieldsHtml = '';
            if (info.fields_detected && info.fields_detected.length > 0) {
                fieldsHtml = `
                    <h6 class="mt-3">📋 Campos Detectados:</h6>
                    <div class="row">
                        ${info.fields_detected.map(field => 
            ...
            let samplesHtml = '';
            if (info.sample_payloads && info.sample_payloads.length > 0) {
                samplesHtml = `
                    <h6 class="mt-3">🔍 Exemplos de Dados:</h6>
                    <div class="accordion" id="sampleAccordion">
                        ${info.sample_payloads.map((payload, index) => `
                            <div class="accordion-item">
                                ...
                                <div id="collapse${index}" class="accordion-collapse collapse" 
                                    ...
                                    <div class="accordion-body">
                                        <pre class="bg-light p-2 rounded"><code>${JSON.stringify(payload, null, 2)}</code></pre>
                                    </div>
                                </div>
                            </div>
                        `).join('')}
                ...
                <div class="card border-success">
                    <div class="card-header bg-success text-white">
                        <h6 class="mb-0">
                            ...
                            Informações da Collection: ${info.name}
                        </h6>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                                <strong>📊 Pontos Vetoriais:</strong><br>
                                <span class="text-primary fs-5">${info.points_count.toLocaleString()}</span>
                            </div>
                            <div class="col-md-3">
                                <strong>🎯 Dimensões:</strong><br>
                                <span class="text-info">${info.config.size}D</span>
                            </div>
                            <div class="col-md-3">
                                <strong>📏 Distância:</strong><br>
                                <span class="text-warning">${info.config.distance.toUpperCase()}</span>
                            </div>
                            <div class="col-md-3">
            ...
            console.error('❌ Erro ao obter informações da collection:', data.error);
            container.innerHTML = `
                <div class="alert alert-danger">
                    <i class="fas fa-exclamation-triangle me-2"></i>
                    ${data.message || 'Erro ao carregar informações da collection'}
                </div>
            `;
        }
    } catch (error) {
        console.error('❌ Erro de rede ao obter informações da collection:', error);
        container.innerHTML = `
            <div class="alert alert-danger">
                <i class="fas fa-exclamation-triangle me-2"></i>
                Erro de conexão ao obter informações da collection
            </div>
        `;
    }
};
...
/**
 * Configura seleção de collection Qdrant com eventos automáticos
 * @param {string} selectId - ID do select de collections
 * @param {string} infoContainerId - ID do container para exibir informações
 */
window.configurarSelecaoQdrant = async function(selectId, infoContainerId) {
    console.log(`🔧 Configurando seleção Qdrant: ${selectId} -> ${infoContainerId}`);
    ...
    console.log(`🔄 Chamando carregarCollectionsQdrant para ${selectId}...`);
    try {
        await window.carregarCollectionsQdrant(selectId);
    } catch (error) {
    ...
    selectElement.addEventListener('change', function() {
        const collectionSelecionada = this.value;
        console.log(`📝 Collection selecionada: ${collectionSelecionada}`);
        if (infoContainerId) {
            window.visualizarInfoCollection(collectionSelecionada, infoContainerId);
        }
    });
};

/**
 * Adiciona seletor de collection Qdrant a um container
 * @param {string} containerId - ID do container onde adicionar
 * @param {Object} config - Configurações do seletor
 */
// ==================== FUNÇÕES UNIVERSAIS DE TREINAMENTO E TESTE ====================

/**
 * Função universal para treinar qualquer tipo de modelo
 * @param {string} tipoModelo - Tipo do modelo (regressao, classificacao, etc.)
 * @param {function} coletarDadosCallback - Callback para coletar dados específicos do modelo
 * @param {function} validarCallback - Callback para validar configuração específica
 */
window.treinarModeloUniversal = function(tipoModelo, coletarDadosCallback, validarCallback) {
    console.log(`🧠 Iniciando treinamento de modelo: ${tipoModelo}`);
    
    // Validar configurações se callback fornecido
    if (validarCallback && !validarCallback()) {
        return;
    }
    
    // Coletar dados específicos do modelo
    const dadosCompletos = coletarDadosCallback ? coletarDadosCallback() : {};
    
    // Mostrar modal de progresso
    mostrarModalTreinamentoUniversal(tipoModelo);
    
    // Enviar para API
    fetch('/api/modelos/treinar', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        ...
        ocultarModalTreinamentoUniversal();
        
        if (data.success) {
            mostrarResultadoTreinamentoUniversal(data.resultado, tipoModelo);
        } else {
            mostrarErro('Erro no treinamento: ' + (data.message || data.error));
        }
    })
    .catch(error => {
        console.error('❌ Erro no treinamento:', error);
        ocultarModalTreinamentoUniversal();
        mostrarErro('Erro de conexão durante o treinamento');
    });
};

/**
 * Função universal para testar qualquer tipo de modelo
 * @param {string} tipoModelo - Tipo do modelo (regressao, classificacao, etc.)
 * @param {function} coletarDadosCallback - Callback para coletar dados específicos do modelo
 * @param {function} validarCallback - Callback para validar configuração específica
 */
window.testarModeloUniversal = function(tipoModelo, coletarDadosCallback, validarCallback) {
    console.log(`🧪 Iniciando teste de modelo: ${tipoModelo}`);
    ...
    // Mostrar modal de progresso
    mostrarModalTesteUniversal(tipoModelo);
    
    // Enviar para API
    fetch('/api/modelos/testar', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        ...
        ocultarModalTesteUniversal();
        
        if (data.success) {
            mostrarResultadoTesteUniversal(data.resultado, tipoModelo);
        } else {
            mostrarErro('Erro no teste: ' + (data.message || data.error));
        }
    })
    .catch(error => {
        console.error('❌ Erro no teste:', error);
        ocultarModalTesteUniversal();
        mostrarErro('Erro de conexão durante o teste');
    });
};

function mostrarModalTreinamentoUniversal(tipoModelo) {
    const modalHtml = `
        <div class="modal fade" id="modalTreinamentoUniversal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title">
                            <i class="fas fa-brain me-2"></i>Treinamento em Progresso
                        </h5>
                    </div>
                    <div class="modal-body text-center">
                        <div class="spinner-border text-success mb-3" style="width: 3rem; height: 3rem;" role="status"></div>
                        <h5>Treinando modelo de ${tipoModelo}...</h5>
                        <p class="text-muted">Este processo pode levar alguns minutos. Aguarde...</p>
                        ...
                        <small class="text-info">
                            ...
                            Processando dados e otimizando hiperparâmetros
                        </small>
                    </div>
                </div>
            </div>
    ...
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    const modal = new bootstrap.Modal(document.getElementById('modalTreinamentoUniversal'));
    modal.show();
}

function ocultarModalTreinamentoUniversal() {
    const modal = bootstrap.Modal.getInstance(document.getElementById('modalTreinamentoUniversal'));
    if (modal) {
        modal.hide();
        setTimeout(() => {
            const modalEl = document.getElementById('modalTreinamentoUniversal');
            if (modalEl) modalEl.remove();
        }, 300);
    }
}

function mostrarModalTesteUniversal(tipoModelo) {
    const modalHtml = `
        <div class="modal fade" id="modalTesteUniversal" tabindex="-1" data-bs-backdrop="static" data-bs-keyboard="false">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">
                            <i class="fas fa-play me-2"></i>Teste em Progresso
                        </h5>
                    </div>
                    <div class="modal-body text-center">
                        <div class="spinner-border text-warning mb-3" style="width: 3rem; height: 3rem;" role="status"></div>
                        <h5>Testando modelo de ${tipoModelo}...</h5>
                        <p class="text-muted">Avaliando performance e gerando métricas...</p>
                        <div class="progress mb-3">
    ...
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    const modal = new bootstrap.Modal(document.getElementById('modalTesteUniversal'));
    modal.show();
}

function ocultarModalTesteUniversal() {
    const modal = bootstrap.Modal.getInstance(document.getElementById('modalTesteUniversal'));
    if (modal) {
        modal.hide();
        setTimeout(() => {
            const modalEl = document.getElementById('modalTesteUniversal');
            if (modalEl) modalEl.remove();
        }, 300);
    }
}

function mostrarResultadoTreinamentoUniversal(resultado, tipoModelo) {
    let metricas = '';
    
    if (tipoModelo === 'regressao') {
        metricas = `
            <tr><td><strong>R² Score:</strong></td><td><span class="badge bg-primary">${resultado.r2_score || 'N/A'}</span></td></tr>
            <tr><td><strong>RMSE:</strong></td><td><span class="badge bg-info">${resultado.rmse || 'N/A'}</span></td></tr>
            <tr><td><strong>MAE:</strong></td><td><span class="badge bg-warning">${resultado.mae || 'N/A'}</span></td></tr>
        `;
    } else if (tipoModelo === 'classificacao') {
        metricas = `
            <tr><td><strong>Accuracy:</strong></td><td><span class="badge bg-primary">${resultado.accuracy || 'N/A'}</span></td></tr>
            <tr><td><strong>Precision:</strong></td><td><span class="badge bg-info">${resultado.precision || 'N/A'}</span></td></tr>
            <tr><td><strong>Recall:</strong></td><td><span class="badge bg-success">${resultado.recall || 'N/A'}</span></td></tr>
            <tr><td><strong>F1-Score:</strong></td><td><span class="badge bg-warning">${resultado.f1_score || 'N/A'}</span></td></tr>
        `;
    } else {
        metricas = `
            <tr><td><strong>Status:</strong></td><td><span class="badge bg-success">${resultado.status || 'Treinado'}</span></td></tr>
        `;
    }
    
    const modalHtml = `
        <div class="modal fade" id="modalResultadoTreinamentoUniversal" tabindex="-1">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header bg-success text-white">
                        <h5 class="modal-title">
                            <i class="fas fa-check-circle me-2"></i>Treinamento de ${tipoModelo} Concluído
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <h6><i class="fas fa-chart-bar me-1"></i>Métricas de Performance</h6>
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        <tbody>
                                            ${metricas}
                                            <tr><td><strong>Tempo de Treinamento:</strong></td><td>${resultado.tempo_treinamento || 'N/A'}</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <h6><i class="fas fa-cogs me-1"></i>Parâmetros Utilizados</h6>
                                <div class="table-responsive">
                                    <table class="table table-sm">
                                        <tbody>
                                            <tr><td><strong>Algoritmo:</strong></td><td>${resultado.algoritmo || 'N/A'}</td></tr>
                                            <tr><td><strong>Amostras de Treino:</strong></td><td>${resultado.amostras_treino || 'N/A'}</td></tr>
                                            <tr><td><strong>Amostras de Teste:</strong></td><td>${resultado.amostras_teste || 'N/A'}</td></tr>
                                            <tr><td><strong>Features:</strong></td><td>${resultado.num_features || 'N/A'}</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-success" onclick="salvarModeloTreinado('${tipoModelo}')">
                            <i class="fas fa-save me-1"></i>Salvar Modelo
                        </button>
                    </div>
                </div>
    ...
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    const modal = new bootstrap.Modal(document.getElementById('modalResultadoTreinamentoUniversal'));
    modal.show();
    
    // Armazenar resultado para uso posterior
    window.ultimoResultadoTreinamento = resultado;
}

function mostrarResultadoTesteUniversal(resultado, tipoModelo) {
    let conteudoResultado = '';
    
    if (tipoModelo === 'regressao') {
        conteudoResultado = `
            <div class="col-md-4">
                <h6><i class="fas fa-bullseye me-1"></i>Métricas de Precisão</h6>
                <div class="card border-primary">
                    <div class="card-body text-center">
                        <h4 class="text-primary">${resultado.r2_score || 'N/A'}</h4>
                        <small>R² Score</small>
                    </div>
                </div>
                <div class="card border-info mt-2">
                    <div class="card-body text-center">
                        <h4 class="text-info">${resultado.rmse || 'N/A'}</h4>
                        <small>RMSE</small>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <h6><i class="fas fa-chart-line me-1"></i>Dados do Teste</h6>
                <ul class="list-group list-group-flush">
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Amostras testadas:</span>
                        <strong>${resultado.amostras_teste || 'N/A'}</strong>
                    </li>
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Tempo de processamento:</span>
                        <strong>${resultado.tempo_teste || 'N/A'}</strong>
                    </li>
                    <li class="list-group-item d-flex justify-content-between">
                        <span>Features utilizadas:</span>
                        <strong>${resultado.num_features || 'N/A'}</strong>
                    </li>
                </ul>
            </div>
            <div class="col-md-4">
                <h6><i class="fas fa-exclamation-triangle me-1"></i>Métricas de Erro</h6>
                <div class="card border-warning">
                    <div class="card-body text-center">
                        <h4 class="text-warning">${resultado.mae || 'N/A'}</h4>
                        <small>MAE</small>
                    </div>
                </div>
                <div class="card border-danger mt-2">
                    <div class="card-body text-center">
                        <h4 class="text-danger">${resultado.mse || 'N/A'}</h4>
                        <small>MSE</small>
                    </div>
                </div>
            </div>
        ...
        conteudoResultado = `
            <div class="col-md-3">
                <div class="card border-primary">
                    <div class="card-body text-center">
                        <h4 class="text-primary">${resultado.accuracy || 'N/A'}</h4>
            ...
            <div class="col-md-3">
                <div class="card border-info">
                    <div class="card-body text-center">
                        <h4 class="text-info">${resultado.precision || 'N/A'}</h4>
            ...
            <div class="col-md-3">
                <div class="card border-success">
                    <div class="card-body text-center">
                        <h4 class="text-success">${resultado.recall || 'N/A'}</h4>
                        <small>Recall</small>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="card border-warning">
                    <div class="card-body text-center">
                        <h4 class="text-warning">${resultado.f1_score || 'N/A'}</h4>
        ...
        conteudoResultado = `
            <div class="col-12">
                <div class="alert alert-success">
                    <h5><i class="fas fa-check-circle me-2"></i>Teste Concluído</h5>
                    <p>Modelo de ${tipoModelo} testado com sucesso!</p>
                    <ul>
                        <li><strong>Amostras testadas:</strong> ${resultado.amostras_teste || 'N/A'}</li>
                        <li><strong>Tempo de processamento:</strong> ${resultado.tempo_teste || 'N/A'}</li>
                        <li><strong>Algoritmo:</strong> ${resultado.algoritmo || 'N/A'}</li>
                    </ul>
                </div>
            </div>
        `;
    ...
    const modalHtml = `
        <div class="modal fade" id="modalResultadoTesteUniversal" tabindex="-1">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header bg-warning text-dark">
                        <h5 class="modal-title">
                            <i class="fas fa-chart-area me-2"></i>Resultado do Teste - ${tipoModelo}
                        </h5>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
                        <button type="button" class="btn btn-primary" onclick="exportarRelatorioTeste('${tipoModelo}')">
    ...
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    const modal = new bootstrap.Modal(document.getElementById('modalResultadoTesteUniversal'));
    modal.show();
    
    // Armazenar resultado para uso posterior
    window.ultimoResultadoTeste = resultado;
}

function salvarModeloTreinado(tipoModelo) {
    const resultado = window.ultimoResultadoTreinamento;
    if (resultado) {
        console.log(`💾 Salvando modelo ${tipoModelo} treinado...`, resultado);
        mostrarSucesso(`Modelo ${tipoModelo} treinado salvo com sucesso!`);
    }
}

function exportarRelatorioTeste(tipoModelo) {
    const resultado = window.ultimoResultadoTeste;
    if (resultado) {
        console.log(`📄 Exportando relatório de teste ${tipoModelo}...`, resultado);
        mostrarSucesso(`Relatório de teste ${tipoModelo} exportado com sucesso!`);
    }
}

// ==================== SELETOR QDRANT ====================

window.adicionarSeletorQdrant = function(containerId, config = {}) {
    const container = document.getElementById(containerId);
    if (!container) {
        console.error('Container não encontrado:', containerId);
        return;
    ...
    const configuracao = {
        selectId: config.selectId || 'collectionQdrant',
        infoId: config.infoId || 'infoCollectionQdrant',
        showInfo: config.showInfo !== false,
        title: config.title || 'Base Vetorial Qdrant',
        description: config.description || 'Selecione uma collection para treinamento vetorial'
    };
    ...
    let html = `
        <div class="card border-warning">
            <div class="card-header bg-warning text-dark">
                <h6 class="mb-0">
                    <i class="fas fa-vector-square me-2"></i>${configuracao.title}
                </h6>
                <small>${configuracao.description}</small>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-12">
                        <label for="${configuracao.selectId}" class="form-label">
                            <i class="fas fa-database me-1"></i>Collection Qdrant
                        </label>
                        <select id="${configuracao.selectId}" class="form-select">
                            <option value="">📦 Carregando collections...</option>
                        </select>
                        <div class="form-text">Selecione a collection vetorial para análise ML</div>
                    </div>
                </div>
    `;
    
    if (configuracao.showInfo) {
        html += `
                <div class="row mt-3">
                    <div class="col-12">
                        <div id="${configuracao.infoId}">
                            <div class="alert alert-info">
                                <i class="fas fa-info-circle me-2"></i>
                                Selecione uma collection para visualizar informações detalhadas
                            </div>
    ...
    // Configurar funcionalidade
    setTimeout(() => {
        window.configurarSelecaoQdrant(configuracao.selectId, configuracao.infoId);
    }, 100);
    
    console.log(`✓ Seletor Qdrant adicionado ao container ${containerId}`);
    return configuracao;
};