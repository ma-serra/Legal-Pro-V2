"""
Agente especialista em Tribunal do Júri para análise de casos criminais.
"""
import os
import json
import logging
...
logger = logging.getLogger('multiagent.agents.tribunal_juri')

class EspecialistaTribunalJuriAgent(BaseJuridicoAgent):
    """
    Agente especialista em Tribunal do Júri e processos criminais.
    
    Este agente analisa questões relacionadas a julgamentos pelo Tribunal do Júri,
    incluindo quesitação, estratégias de acusação e defesa, nulidades processuais,
    e procedimentos específicos deste rito processual.
    """
    
    def _processar(self, data):
        """
        Processa análises relacionadas ao Tribunal do Júri.
        ...
        try:
            ...
            return {
                ...
                "tipo_documento": "tribunal_juri"
            }
            
        except Exception as e:
            logger.error(f"Erro ao analisar caso do Tribunal do Júri: {str(e)}")
            return {
    ...
    def _get_system_prompt(self, nivel_detalhe):
        ...
        # Base prompt específico para análise de casos do Tribunal do Júri
        prompt = """Você é um especialista em Direito Processual Penal e Tribunal do Júri brasileiro, com mais de 20 anos de experiência acadêmica e profissional, incluindo casos complexos de homicídio qualificado e latrocínio.
...
2. Responda de forma técnico-jurídica, utilizando a nomenclatura exata presente no Código de Processo Penal
3. Identifique os dispositivos legais específicos aplicáveis (artigos do CPP e decisões colegiadas do STF/STJ)
...
1. Questão jurídica principal, extraída diretamente do texto (quesitação, procedimento, nulidades, provas, etc.)
2. Fase processual atual, mencionada ou deduzível diretamente do documento (instrução, plenário, recurso)
3. Análise técnica dos dispositivos legais específicos aplicáveis (artigos do CPP) relacionados às questões apresentadas
4. Nulidades processuais identificadas no texto, com fundamentação legal expressa
5. Jurisprudência vinculante específica (número dos julgados) do STF/STJ aplicável ao caso concreto
6. Avaliação técnica das teses mencionadas no documento, usando terminologia processual penal precisa

IMPORTANTE: Responsa SOMENTE com base no conteúdo do documento. Se algo não estiver explícito no texto, indique claramente que 'o documento não fornece informações suficientes sobre [tema]' em vez de presumir detalhes."""
...
- Detalhamento técnico dos potenciais vícios procedimentais
- Discussão das diferentes correntes doutrinárias sobre o tema
- Análise estatística de sucesso das teses em casos similares
        ...
        if formato == 'html':
            ...
            for paragrafo in analise.split('\n\n'):
                if paragrafo.strip():
                    ...
                    if paragrafo.strip()[0].isdigit() or any(keyword in paragrafo.lower() for keyword in ['questão jurídica', 'fase processual', 'análise técnica', 'nulidades', 'jurisprudência', 'estratégias', 'avaliação']):
            ...
            <div class="analise-juridica analise-tribunal-juri">
                <h2>Análise Jurídica - Tribunal do Júri</h2>
                {html}
            </div>
            """
            ...
            resultado_formatado = f"""
# Análise Jurídica - Tribunal do Júri
